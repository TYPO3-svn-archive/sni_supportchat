...
var chatMarket = new Class({
	...
	"getChat": function(uid) {
		/* returns the reference to the chat with uid */
		var theItem = null;
			...
			switch (handle) {
				case 'fromDoAll':
					...
					if(root[0].childNodes[1]) {
						...
						if(chats.length>0) {
							for (var i=0; i<chats.length; i++) {
								...
								var theChat = this.getChat(uid) || false;
								if(theChat) {
									...
									var addInfo = item.getElementsByTagName("additionalInfo");
									var data = "";
									if(addInfo[0]) {
										// only if Hook in lib/class.tx_chatMarket.php is used!
										data = this.getTextNode(addInfo[0].firstChild);
									}
								...
								theChat.draw(); // draw the chat (messages or whole Chat, depends..)
								var from_lock_chat = item.getElementsByTagName("from_lock_chat");
								if(from_lock_chat[0]) {
									// last request was also a lockChat request
									var code = from_lock_chat[0];
									var locked = this.getTextNode(code.firstChild);
									theChat.removeLoadingImg();
									theChat.lockChatDone(locked);
								}
					...
					if(root[0].childNodes[2]) {
						var logMsg = root[0].childNodes[2].getElementsByTagName("numIndex");
						if(logMsg.length > 0) {
					...
					/* watch if chats should be removed, if for example an other BE-User has locked the chat */
					if(chats) {
						this.chats.each(function(item,index) {
							if(item) {
								var toDelete = 1;
								for (var i=0; i<chats.length; i++) {
									if(item.uid == chats[i].childNodes[0].firstChild.nodeValue) {
										toDelete = 0;
									}
								}
								if(toDelete) {
	...
	"removeChat": function(theChat) {
		/* destroys a chat local (HTML and JS Object) */
		theChat.removeChat();
		this.chats.erase(theChat);
	}
});
...
var chat = new Class({
	...
	"initialize": function(uid,useTypingIndicator,crdate,language,be_user,lastRow,surferIp,lImg,lLabel,additionalInfo,options) {
	...
	"draw": function() {
		...
		if(!this.chatVisible) {
			...
			wrap.adopt(topDiv,title,chatbox,textBoxLabel,textArea,assumeIcon,typingIcon,status); /*added for typingStatus*/
